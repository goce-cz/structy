...
const generatePipeOverload = (argCount: number) => {
    const args = new Array(argCount).fill(null).map((_, i) => String.fromCharCode(A_ASCII + i));
    return [
        ...
        [
            'a: BoundOptic<A>',
            ...args.slice(1).map((arg, index) => `${args[index]}${arg}: (${args[index]}: ${args[index].toUpperCase()}) => ${arg.toUpperCase()}`)
        ].join(', '),
